...

{
    ...
    public class DataStore
    {
        ...
        private static readonly string filepath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "MyCinemaApp", "MyData.json");

        private readonly object _locker = new();

        public JsonSerializerOptions _ops = new() { WriteIndented = true };
        public CinemaData Data { get; private set; } = new();
        ...
        public void Load()
        {
            ...
            lock (_locker)
            {
                ...
                {
                    Console.WriteLine($"Sorry fiald to load the data : {ex.Message}");
                    Data = new CinemaData();
                }
            }
        }
        public void Save()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(dir))
                {
                    ...
                    dir = Path.Combine(appData, "MyCinemaApp");
                }
                ...
                var temp = Path.Combine(dir, Path.GetFileName(filepath) + ".tmp");
            ...
            {
                Console.Error.WriteLine($"Save failed. FilePath='{filepath}'. Error: {ex.GetType().Name}: {ex.Message}");
            }
        }
